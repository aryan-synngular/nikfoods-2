...
export default function FoodCategory() {
 ...
 const limit=7;
   const getCategories = useCallback(async () => {
     try {
       setLoading(true);
       const response = await fetch(`/api/food-category?search=${search}&page=${page}&limit=${limit}`);
 ...
 console.log(deleteItemId)
   ...
   const deleteCategory=useCallback(async()=>{
    ...
    return <YStack space="$5" p="$4">
          <XStack items="center" justify="space-between" mb="$3" gap={16}>
            <XStack items="center"  gap={32}>
             
            <Text fontWeight="bold" fontSize={20} >Food Category</Text>
              
            </XStack>
                    <Button size="$3"  color="white" hoverStyle={{bg:"#4F8CFF"}}  bg={"#4F8CFF"} p={12}  icon={Plus} onPress={() => { setEditDialogOpen(true); setEditItem(null) }}>Add Food Category</Button>
              ...
              <Dialog.Portal>
                ...
                <Dialog.Content style={{ background: '#fff', borderRadius: 12, padding: 24, width: 540, maxWidth: '90vw' }}>
                  <EditCategoryForm
                    ...
                    onSuccess={() => { setEditDialogOpen(false); getCategories() ; }}
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
              <AlertDialog.Portal>
                ...
                <AlertDialog.Content style={{ background: '#fff', borderRadius: 12, padding: 24, width: 440, maxWidth: '90vw' }}>
                  <AlertDialog.Description mb={"$4"}>Are you sure you want to delete this category?</AlertDialog.Description>
                 <XStack gap="$3" justify="flex-end">
                    <AlertDialog.Cancel asChild>
                      <Button>Cancel</Button>
                    </AlertDialog.Cancel>
                    <AlertDialog.Action asChild>
                      <Button onPress={() => { setDeleteDialogOpen(false);deleteCategory() }} theme="accent">Accept</Button>
                    </AlertDialog.Action>
                  </XStack>
                </AlertDialog.Content>
              </AlertDialog.Portal>
