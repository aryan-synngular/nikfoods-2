...
export function FoodListShimmerLoader({ count = 16 }: { count?: number }) {
  ...
  return (
    <YStack style={{ paddingTop:(isMobile||isMobileWeb)?0: 20, paddingBottom: 20 }}>
      <XStack
        flexWrap="wrap"
        gap={(isMobile||isMobileWeb)?4:24}
        style={{ justifyContent: 'flex-start', paddingHorizontal: (isMobile||isMobileWeb)?0:20 }}
      >
        {Array.from({ length: cardCount }).map((_, i) => (
          <YStack key={i} width={(isMobile||isMobileWeb)?190:200} style={{ alignItems: 'center', marginBottom: 16 }}>
            <YStack width={180} style={{ alignItems: 'center' }}>
              <Circle
                size={(isMobile||isMobileWeb)?80:100}
                overflow="hidden"
                style={{
                  elevation: 4,
                  shadowColor: '#000',
