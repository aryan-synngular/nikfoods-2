...
function PaymentFormInner({
  ...
  onOrderCreated,
  orderCalculations,
  clientSecret, // 👈 now passed from parent
  setPaymentStatus,
  setCompletedOrderId,
}: any) {
  ...
  const handleExpressCheckoutConfirm = useCallback(
    async (event: any) => {
      ...
      try {
        ...
        // confirm payment using provided clientSecret
...
export default function PaymentPageWeb(props: any) {
  ...
  // call your backend to create a PaymentIntent when page loads
  useEffect(() => {
    const init = async () => {
      const resp: any = await apiCheckout({
        amount: 1000, // or pass order total from props
      })
  ...
  if (!clientSecret) return <Text>Loading payment...</Text>
